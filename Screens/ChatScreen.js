...
import { useChat } from '../context'; // Import from centralized context file

const ChatScreen = ({ navigation }) => {
  // Get matches and chat data from context
  const { matches } = useMatch();
  const { chats, addMessage } = useChat(); // This should now work correctly
  ...
  const handleSendMessage = () => {
    ...
    // Use the context function to add a message
    ...
    setTimeout(() => {
      const responses = [
        ...
        `I appreciate your interest in my services. Let me know how I can help!`,
        `That sounds interesting! Can you provide more details about your project?`,
        `I'm available to discuss this further. What's your timeline?`
      ];
      const randomResponse = responses[Math.floor(Math.random() * responses.length)];
      addMessage(selectedFreelancer.id, randomResponse, false);
    }, 1000);
  };
  ...
  const renderFreelancerItem = ({ item }) => {
    // Get the latest message for this freelancer
  ...
  const renderChatMessage = ({ item }) => (
    <View
      style={[
        ...
        item.isUser ? styles.userMessage : styles.freelancerMessage,
      ]}
    >
      <Text
        style={[
          styles.messageText,
          item.isUser ? styles.userMessageText : styles.freelancerMessageText,
        ]}
      >
        {item.text}
      </Text>
      <Text
        style={[
          styles.timestamp,
          item.isUser ? styles.userTimestamp : styles.freelancerTimestamp,
        ]}
      >
